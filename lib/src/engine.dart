...
class FocusEngine {
  ...
  List<double> _zscoreNormalize(List<double> values) {
    ...
    final mean = _subjectIbiHistory.reduce((a, b) => a + b) / _subjectIbiHistory.length;
    final variance = _subjectIbiHistory
        .map((x) => math.pow(x - mean, 2))
        .reduce((a, b) => a + b) / _subjectIbiHistory.length;
  ...
  Future<FocusResult?> inferFromHrData({
    ...
    _hrBuffer.removeWhere((v) => timestamp.difference(v.timestamp) > windowDuration);
    ...
    final bufferDurationTotalSeconds = bufferDuration.inMilliseconds / 1000.0; // More precise
    
    // Also check time since first data point was collected
    final timeSinceFirstData = timestamp.difference(_firstDataTime!);
    final timeSinceFirstDataSeconds = timeSinceFirstData.inMilliseconds / 1000.0;
    ...
    if (_lastWindowTime == null) {
      ...
      if (timeSinceFirstDataSeconds < config.windowSeconds) {
        ...
        _log('debug', 'Collecting data: ${timeSinceFirstDataSeconds.toStringAsFixed(2)}s / ${config.windowSeconds}s (${_hrBuffer.length} points, buffer span: ${bufferDurationTotalSeconds.toStringAsFixed(2)}s)');
        return null;
      }
      // We have at least 60 seconds since first data, do first inference
      _log('info', 'First inference: 60-second window complete (${_hrBuffer.length} data points, elapsed: ${timeSinceFirstDataSeconds.toStringAsFixed(2)}s, buffer span: ${bufferDurationTotalSeconds.toStringAsFixed(2)}s)');
    } else {
      ...
      _log('debug', 'Sliding window inference: processing last 60 seconds (${_hrBuffer.length} data points, ${bufferDurationTotalSeconds.toStringAsFixed(2)}s)');
    }
    
    // Check minimum data requirement
    if (_hrBuffer.length < config.minRrCount) {
      _log('warning', 'Insufficient data points: ${_hrBuffer.length} < ${config.minRrCount}');
      return null;
    }
    ...
    _log('info', 'Proceeding with inference: buffer has ${_hrBuffer.length} points spanning ${bufferDurationTotalSeconds.toStringAsFixed(2)}s (elapsed: ${timeSinceFirstDataSeconds.toStringAsFixed(2)}s)');
